...
Summary:
    ...
    Laura asked me to ignore all the bigrams and try to see what bi-gramas
    tri-grams and n grams are meaningful and is searchable to wikipedia
    concepts
Input:
...
for doc_id, list_of_concepts in phrases.items():
    ...
    if type(list_of_concepts) != float:
        ...
        non_unigram_concepts = [concept for concept,val in list_of_concepts if get_ngram_size(concept) > 2]
        for concept in non_unigram_concepts: concept_count[concept] += 1 
...
# ax.set_xticklabels(val,rotation=90)
# ax.set_ylabel("Count")
# ax.set_xlabel("Count of how many n-gram Count")
...
# #plt.savefig("../../figures/%s_data_mgt_change_in_rank_bucket_histogram_%s.png" %(output_code,rank_type), dpi = 150)
# plt.show()
...
writelines = []
writelines.append(["non-unigram concept", "number of different times appeared"])
